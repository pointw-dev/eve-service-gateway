...
If using auth (e.g. `eve-utils api create eve-service-gateway --add_auth` )
...
| ES-AUTH_REALM          | Appears in the `WWW-Authenticate` header in unauthorized requests. | eve-service-gateway.pointw.com                       |
| ES-AUTH_JWT_DOMAIN     |                                                              | eve-service-gateway.us.auth0.com                     |
| ES-AUTH_JWT_AUDIENCE   | This is the identifier a client uses when requesting a token from the auth provider.  It is a URI only (identifier only), not an actual URL (i.e. no requests are made to it) | https://pointw.com/eve-service-gateway               |
| AUTH0_API_AUDIENCE     | When eve-service-gateway requests a token to use the Auth0 API, this is the audience for the token. | https://eve-service-gateway.us.auth0.com/api/v2/     |
| AUTH0_API_BASE_URL     | The base of the Auth0 API                                    | https://eve-service-gateway.us.auth0.com/api/v2      |
| AUTH0_CLAIMS_NAMESPACE | If you configure Auth0 to insert additional claims, use this value as a namespace (prefix). | https://pointw.com/eve-service-gateway               |
| AUTH0_TOKEN_ENDPOINT   | When eve-service-gateway needs to call the Auth0 API, it uses this endpoint to request a token. | https://eve-service-gateway.us.auth0.com/oauth/token |
| AUTH0_CLIENT_ID        | When eve-service-gateway needs to call the Auth0 API, it uses this client id/secret to authenticate.  These are not the client id/secret of your application. | --your-client-id--                               |
